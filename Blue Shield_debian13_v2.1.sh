#!/usr/bin/env bash
# =============================================================================
# BlueShield Debian 13 — Hardening Completo e Interactivo  ·  v2.1
# -----------------------------------------------------------------------------
...
#   · blueshield-clamscan.service endurecido con directivas systemd coherentes
...
#     envoltorio 'blueshield-sandbox', Flatpak + Flathub + Flatseal y un script
...
BACKUP_DIR="/root/blueshield-backups/$(date +%F-%H%M%S)"
mkdir -p "$BACKUP_DIR"
LOGFILE="$BACKUP_DIR/blueshield.log"
: > "$LOGFILE"
...
cat > /etc/apt/apt.conf.d/51blueshield-unattended <<EOF
// BlueShield: comportamiento de unattended-upgrades
...
cat > /etc/systemd/journald.conf.d/99-blueshield.conf <<'EOF'
[Journal]
Storage=persistent
Compress=yes
SystemMaxUse=1G
...
SYSCTL_H="/etc/sysctl.d/99-blueshield.conf"
[[ -f "$SYSCTL_H" ]] && cp -a "$SYSCTL_H" "$BACKUP_DIR/99-blueshield.conf.bak"

cat > "$SYSCTL_H" <<EOF
# ── BlueShield: Hardening de kernel y red ─────────────────────
# Generado por blueshield v2.1 · perfil: $PROFILE
...
SSHD_DROPIN="$SSHD_DIR/99-blueshield.conf"
mkdir -p "$SSHD_DIR"
cp -a /etc/ssh/sshd_config "$BACKUP_DIR/sshd_config.bak"
[[ -f "$SSHD_DROPIN" ]] && cp -a "$SSHD_DROPIN" "$BACKUP_DIR/99-blueshield-sshd.conf.bak"
...
# ── Configuración ──
{
  echo "# BlueShield v2.1 — generado $(date -Is)"
  echo "Port ${SSH_PORT}"
...
table inet blueshield
delete table inet blueshield"
...
cat > "$NFT_CONF" <<EOF
#!/usr/sbin/nft -f
# BlueShield v2.1 · perfil: $PROFILE · generado $(date -Is)
...
${DESKTOP_RULES}${SSH_ALLOW_RULES}${EXTRA_RULES}
    ...
    limit rate 5/minute burst 10 packets log prefix "blueshield-drop: " level info
  }
...
info "Reglas activas:"
result_block "$(nft list table inet blueshield 2>&1 | head -50 || true)"
...
if [[ "$ENABLE_SSH" == "1" ]]; then
  ...
  cat > /etc/fail2ban/jail.d/blueshield.local <<EOF
[DEFAULT]
backend   = systemd
...
AUDIT_RULES="$AUDIT_RULES_D/99-blueshield.rules"
mkdir -p "$AUDIT_RULES_D"
[[ -f "$AUDIT_RULES" ]] && cp -a "$AUDIT_RULES" "$BACKUP_DIR/99-blueshield.rules.bak"
...
{
cat <<EOF
## BlueShield v2.1 — reglas de auditoría · perfil: $PROFILE · nivel: $AUDIT_LEVEL
...
QUARANTINE_DIR="/var/lib/blueshield/cuarentena"
CLAM_TMPDIR="/var/lib/blueshield/tmp"
...
cat > /usr/local/sbin/blueshield-clamscan <<EOF
#!/bin/bash
# Escaneo programado — BlueShield v2.1
LOG=/var/log/blueshield-clamscan.log
{
  echo "=== \$(date -Is) — inicio ==="
...
EOF
chmod 750 /usr/local/sbin/blueshield-clamscan
...
cat <<'EOF'
[Unit]
Description=BlueShield - escaneo ClamAV programado
Documentation=man:clamscan(1)

[Service]
Type=oneshot
ExecStart=/usr/local/sbin/blueshield-clamscan
Nice=19
...
ProtectHome=no
ReadWritePaths=/var/log /var/lib/blueshield /home /srv /tmp /var/tmp
EOF
else
cat <<'EOF'
...
ReadWritePaths=/var/log /var/lib/blueshield/tmp
ReadOnlyPaths=/srv
EOF
fi
...
EOF
} > /etc/systemd/system/blueshield-clamscan.service

cat > /etc/logrotate.d/blueshield-clamscan <<'EOF'
/var/log/blueshield-clamscan.log {
    monthly
    rotate 6
    compress
    delaycompress
...
cat > /etc/systemd/system/blueshield-clamscan.timer <<'EOF'
[Unit]
Description=BlueShield - escaneo ClamAV semanal
...
if systemd-analyze verify /etc/systemd/system/blueshield-clamscan.service >>"$LOGFILE" 2>&1; then
  ok "Unidad blueshield-clamscan.service validada."
else
  warn "systemd-analyze verify emitió avisos — revisa $LOGFILE"
fi

systemctl enable --now blueshield-clamscan.timer >>"$LOGFILE" 2>&1
ok "Escaneo semanal programado (domingos 03:00)."

if command -v systemd-analyze >/dev/null 2>&1; then
  SEC_SCORE=$(systemd-analyze security blueshield-clamscan.service --no-pager 2>/dev/null | tail -3 || true)
...
info "Prueba manual del escaneo:  systemctl start blueshield-clamscan.service"
info "Resultado:                  tail -f /var/log/blueshield-clamscan.log"
...
# ── 15.1 · bubblewrap + envoltorio blueshield-sandbox ──
if command -v bwrap >/dev/null 2>&1; then
  cat > /usr/local/bin/blueshield-sandbox <<'SANDBOX'
#!/usr/bin/env bash
# =============================================================================
# blueshield-sandbox — ejecuta un programa en un contenedor bubblewrap desechable
# -----------------------------------------------------------------------------
# Uso:
#   blueshield-sandbox [opciones] COMANDO [ARGS...]
#
# Opciones:
...
SBHOME=$(mktemp -d "${TMPDIR:-/tmp}/blueshield-sandbox.XXXXXXXX")
...
exec bwrap \
  ...
  -- "$@"
SANDBOX
  chmod 755 /usr/local/bin/blueshield-sandbox
  ok "Envoltorio instalado: /usr/local/bin/blueshield-sandbox"
  info "Abrir un PDF de origen dudoso sin red y con HOME desechable:"
  info "    blueshield-sandbox --ro ~/Descargas/x.pdf evince ~/Descargas/x.pdf"
  info "Ayuda completa:  blueshield-sandbox --help"
else
  warn "bwrap no está disponible — se omite el envoltorio blueshield-sandbox."
fi
...
if [[ "$INSTALL_FLATPAK" == "1" ]]; then
  ...
  cat > /usr/local/sbin/blueshield-flatpak-lockdown <<'LOCKDOWN'
#!/usr/bin/env bash
# =============================================================================
# blueshield-flatpak-lockdown — recorta los permisos GLOBALES de Flatpak
# -----------------------------------------------------------------------------
...
LOCKDOWN
  chmod 750 /usr/local/sbin/blueshield-flatpak-lockdown
  ok "Script de recorte disponible: /usr/local/sbin/blueshield-flatpak-lockdown"

  if [[ "$FLATPAK_LOCKDOWN" == "1" ]]; then
    warn "FLATPAK_LOCKDOWN=1 — aplicando el recorte global ahora."
    if /usr/local/sbin/blueshield-flatpak-lockdown >>"$LOGFILE" 2>&1; then
    ...
    info "Ejecútalo cuando quieras:  sudo blueshield-flatpak-lockdown"
  fi
else
...
info "  · blueshield-sandbox     → cualquier binario, HOME desechable, sin red"
...
if [[ -d /etc/aide/aide.conf.d ]]; then
  cat > /etc/aide/aide.conf.d/99-blueshield <<'EOF'
# BlueShield: exclusiones para reducir ruido
!/home
!/root/blueshield-backups
!/var/log
!/var/lib/blueshield
!/var/cache
!/var/tmp
...
!/var/lib/systemd
EOF
  ok "Exclusiones escritas en /etc/aide/aide.conf.d/99-blueshield"
  if [[ "$PROFILE" == "workstation" ]]; then
...
result_block "$(nft list chain inet blueshield input 2>&1 | head -30 || true)"
...
printf "\n\033[1;32m  BlueShield v2.1 completado.\033[0m\n\n"
